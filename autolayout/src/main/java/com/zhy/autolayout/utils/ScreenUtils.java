...
// since SDK_INT = 1;
        ...
        Log.d("ScreenUtils", "heightPixels1:" + heightPixels);
        if (!useDeviceSize) {
            size[0] = widthPixels;
...
// includes window decorations (statusbar bar/menu bar)
        if (Build.VERSION.SDK_INT >= 17)
            try {
                ...
                Log.d("ScreenUtils", "heightPixels:" + heightPixels);
            } catch (Exception ignored) {
            }
        size[0] = widthPixels>0?widthPixels:1080;
        size[1] = heightPixels>0?heightPixels:1920;
        return size;
    }
